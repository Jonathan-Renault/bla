...
class FW_Extension_Sign_Form extends FW_Extension {
	...
	public function registerLinkSC( $atts ) {
		...
		$url	 = $atts[ 'url' ] ? $atts[ 'url' ] : esc_url( '' );
		$text	 = $atts[ 'text' ] ? $atts[ 'text' ] : esc_html__( 'Register Now!', 'crum-ext-sign-form' );

		return "<a id='register-now' data-target='#registration-login-form-popup' 
                data-toggle=\"modal\" href='{$url}'>{$text}</a>";
	}
...
//        check_ajax_referer( 'crumina-sign-form' );
		...
		if ( !$log ) {
			$errors[ 'log' ] = esc_html__( 'E-mail requis', 'crum-ext-sign-form' );
		}

		if ( !$pwd ) {
			$errors[ 'pwd' ] = esc_html__( 'Mot de passe requis', 'crum-ext-sign-form' );
		}
...
//		$last_name	 = filter_input( INPUT_POST, 'last_name' );
		...
		if ( !$user_login ) {
			$errors[ 'user_login' ] = esc_html__( 'Nom d\'utilisateur requis', 'crum-ext-sign-form' );
		}

		if ( !$user_email ) {
			$errors[ 'user_email' ] = esc_html__( 'E-mail requis', 'crum-ext-sign-form' );
		}
//
//		if ( !$first_name ) {
...
//		}
		...
		if ( strlen( $user_password ) < 6 ) {
			$errors[ 'user_password' ] = esc_html__( 'Le mot passe doit faire un minimum de 6 charactères', 'crum-ext-sign-form' );
		} else if ( $user_password !== $user_password_confirm ) {
			$errors[ 'user_password_confirm' ] = esc_html__( 'Les mots de passe ne correspondent pas', 'crum-ext-sign-form' );
		}

		if ( !$gdpr ) {
			$errors[ 'gdpr' ] = esc_html__( 'Veuillez accepter les conditions d\'utilisations', 'crum-ext-sign-form' );
		}
	...
	public static function register_new_user( $user_login, $user_email, $user_pass ) {
		...
		if ( $sanitized_user_login === '' ) {
			$errors->add( 'empty_username', __( '<strong>ERREUR</strong>: Veuillez entrer un nom d\'utilisateur.' ) );
		} elseif ( !validate_username( $user_login ) ) {
			$errors->add( 'invalid_username', __( '<strong>ERREUR</strong>: Ce nom d\'utilisateur contient des charactères non autorisés. 
														Veuillez entrer un nom d\'utilisateur valide.' ) );
			$sanitized_user_login = '';
		} elseif ( username_exists( $sanitized_user_login ) ) {
			$errors->add( 'username_exists', __( '<strong>ERREUR</strong>: Ce nom d\'utilisateur est déjà utilisé' ) );
		} else {
			/** This filter is documented in wp-includes/user.php */
			$illegal_user_logins = array_map( 'strtolower', (array) apply_filters( 'illegal_user_logins', array() ) );
			if ( in_array( strtolower( $sanitized_user_login ), $illegal_user_logins ) ) {
				$errors->add( 'invalid_username', __( '<strong>ERREUR</strong>: Désolé, ce nom d\'utilisateur n\'est pas autorisé.' ) );
			}
		}

		// Check the email address
		if ( $user_email === '' ) {
			$errors->add( 'empty_email', __( '<strong>ERREUR</strong>: Veuillez entrer une adresse e-mail.' ) );
		} elseif ( !is_email( $user_email ) ) {
			$errors->add( 'invalid_email', __( '<strong>ERREUR</strong>: L\'adresse e-mail n\'est pas valide.' ) );
			$user_email = '';
		} elseif ( email_exists( $user_email ) ) {
			$errors->add( 'email_exists', __( '<strong>ERREUR</strong>: Cette adresse e-mail est déja utilisée.' ) );
		}

		// Check the password
		if ( $user_pass === '' ) {
			$errors->add( 'empty_pass', __( '<strong>ERREUR</strong>: Veuillez entrer un mot de passe.' ) );
		} elseif ( strlen( $user_pass ) < 6 ) {
			$errors->add( 'invalid_pass', __( '<strong>ERREUR</strong>: Le mot passe doit faire un minimum de 6 charactères.' ) );
		}
		...
		if ( !$user_id || is_wp_error( $user_id ) ) {
			$errors->add( 'registerfail', sprintf( __( '<strong>ERREUR</strong>: Couldn&#8217;t register you&hellip; please contact the <a href="mailto:%s">webmaster</a> !' ), get_option( 'admin_email' ) ) );
			return $errors;
		}
	...
	public static function getPrivacyLink() {
		...
		return esc_html__( 'Accepter les conditions d\'utilisations', 'crum-ext-sign-form' );
	}
