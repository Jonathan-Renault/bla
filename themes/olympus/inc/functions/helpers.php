...
if ( !function_exists( 'olympus_is_composer' ) ) {
	 ...
	 * @package olympus-wp
	 */
	function olympus_is_composer() {
		$vc_js_status = (string) get_post_meta( get_the_ID(), '_wpb_vc_js_status', true );

		return ( $vc_js_status === 'true' || $vc_js_status === '1' ) ? true : false;
	}
...
if ( !function_exists( 'olympus_generate_font_styles' ) ) {
	...
	function olympus_generate_font_styles( $tag ) {
		...
		if ( 'nav' === $tag ) {
			$font_css .= '.left-menu-icon{color:' . $font_color . '; fill:' . $font_color . ';}';
			$font_css .= '.left-menu-title,'
...
if ( !function_exists( 'olympus_svg_icon' ) ) {
	...
	function olympus_svg_icon( $icon_id, $extra_class = '', $atts = '' ) {
		return '<svg class="' . esc_attr( $icon_id ) . ' ' . esc_attr( $extra_class ) . '" ' . $atts . '><use xlink:href="' . get_template_directory_uri() . '/images/icons.svg#' . esc_attr( $icon_id ) . '"></use></svg>';
	}
			...
			<div class="post-thumb bg-link"
				...
				<div class="post-content">
					...
					<a href="<?php echo esc_url( $post_url ); ?>" class="post-link" rel="nofollow" target="_blank">
						<?php echo olympus_svg_icon( 'olymp-link-icon' ); ?>
					</a>
...
if ( !function_exists( 'olympus_prepare_megamenu_icon_params' ) ) {
	 ...
	 * @return array
	 */
	function olympus_prepare_megamenu_icon_params( $meta = '' ) {
			...
			<?php foreach ( $links as $link ) : ?>
				<li class="menu-item menu-item-has-icon">
					<a href="<?php echo esc_url( $link[ 'href' ] ); ?>">
						...
						<use xlink:href="<?php echo get_template_directory_uri(); ?>/images/icons.svg#<?php echo esc_attr( $link[ 'icon' ] ); ?>"></use>
						</svg>
...
if ( !function_exists( 'olympus_query_posts' ) ) {
	...
	function olympus_query_posts() {
		...
		$per_page	 = $olympus->get_option_final( 'posts_per_page', 12 );
...
function olympus_is_left_panel_visible() {
	$olympus = Olympus_Options::get_instance();
	$visible = $olympus->get_option_final( 'left-panel-fixed-options/show', 'no' );
...
function olympus_is_top_menu_panel_visible() {
	$olympus = Olympus_Options::get_instance();
	$visible = $olympus->get_option_final( 'top-menu-panel-options/show', 'yes' );
