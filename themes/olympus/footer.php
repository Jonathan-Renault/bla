...
if ( 'landing-template.php' !== get_page_template_slug() ) {
	...
	$to_top_options	 = $olympus->get_option( 'scroll_top_icon', array() );
	$to_top_show	 = olympus_akg( 'value', $to_top_options, 'show' );

	$description_options	 = $olympus->get_option( 'site_description', false );
	...
	$footer_copyright	 = $olympus->get_option(
			'footer_copyright', 'Copyright &copy; ' . date( "Y" ) . ' <a href="' . esc_url( $my_theme->get( 'AuthorURI' ) ) . '"></a>Premium WordPress theme ' . esc_url( $my_theme->get( 'Theme Name' ) )
	);
	...
	<!-- Footer -->
	<section class="<?php echo esc_attr( implode( ' ', $footer_class ) ) ?>" id="footer">
	    <div class="<?php echo esc_attr( $footer_wide_content ); ?>">
			...
			<?php if ( is_active_sidebar( 'sidebar-footer' ) ) { ?>
				<div class="<?php echo esc_attr( $sidebar_columns ); ?>">
					<div class="row">
						...
						$output					 = ob_get_clean();
						$footer_sibebar_columns	 = $olympus->get_option( 'footer-widget-block-columns', 4 );
			...
			<?php if ( 'show' === $to_top_show ) { ?>
				<span class="smooth-scroll"><a id="back-to-top" data-scroll class="back-to-top" href="#">
					<?php echo olympus_svg_icon( 'arrow-to-top', 'back-icon' ) ?>
				</a></span>
			<?php } ?>
	    </div>
	</section>
	...
	<?php get_template_part( 'templates/user/popup', 'sign' ); ?>
