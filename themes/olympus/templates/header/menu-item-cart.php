...
<li class="cart-menulocation">
	<a href="<?php echo wc_get_cart_url(); ?>" class="nav-link" title="<?php esc_attr_e( 'View your shopping cart', 'olympus' ); ?>">
		<?php echo olympus_svg_icon( 'olymp-shopping-bag-icon' ); ?>
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<li class="cart-product-item">
								...
								if ( $image ) {
									...
									$thumb_html = olympus_html_tag( 'a', array( 'href' => $product_permalink ), '<img src="' . esc_url( $thumb ) . '" alt="' . esc_attr( $product_name ) . '">' );
